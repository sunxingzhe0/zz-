        ...
        <Row v-if="!flagType">
          <Col span="6">
            <FormItem label="合集类型：">
                ...
                <Button @click="videoButtonClick(1)" :type="payButtonType===1?'primary':'default'" style="margin-right:10px">新建</Button>
                <Button @click="videoButtonClick(0)" :type="payButtonType===0?'primary':'default'">修改</Button>
            </FormItem>
          </Col>
    ...
    <div class="table">
      ...
      <Table ref="selection" border :columns="videoColumns" :data="videoData">
        ...
        <template slot-scope="{ row, index }" slot="new">
            <span v-if="row.title===''||row.title===row.title_new">新建</span>
             <span v-if="row.title!==''&&row.title!==row.title_new">修改</span>
          </template>
        <!-- 审核状态 -->
...
export default {
  ...
  methods: {
    ...
    getCombineAuditLis(){
      ...
      collectionTotalApi({
        ...
        this.videoData = res.data.list
         this.videoData =  this.videoData.forEach(item=>{
    ...
    confirmResonValue() {
      ...
      let reason =this.refuseForm.reason === "自定义"? this.refuseVlue: this.refuseForm.reason;
