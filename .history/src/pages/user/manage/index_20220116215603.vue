          ...
          <div style="flex: 0 0 50%">
            ...
            <Row>
              <Col span="18"
                >喜欢视频的类型：
                <span
                  v-if="manageForm.vedio_cate_str"
                  v-html="manageForm.vedio_cate_str"
                  >{{ manageForm.vedio_cate_str }}</span
                ><span v-else>暂无数据</span></Col
              >
            </Row>
          </div>
          <div>
      ...
      <Form ref="formInline" :model="uploadVideoForm" style="margin-top: 10px">
        ...
        <div class="data_warp_box">
          ...
          <div class="uploadVideo textColor">
            ...
            <Input
              style="width: 140px"
              v-model="uploadCover.cover_title"
              :maxlength="7"
              placeholder="最多7个字"
            ></Input
            >&emsp;
          ...
          <div class="uploadVideo textColor">
            ...
            <Input
              style="width: 140px"
              v-model="uploadCover.cover_index"
              @on-keyup="numberBlur(uploadCover.coverNum)"
              number
              placeholder="只允许输入数字"
            ></Input
            >&emsp;
...
export default {
  ...
  methods: {
    ...
    handleBeforeUploadImg(file) {
      ...
      axios({
        ...
        if (res.data.status === 400) {
          ...
          this.uploadVideoForm.picUrl = Setting.url(
            this.uploadVideoForm.picUrl
          );
          console.log(
            "视频在元数据加载完成时得到的视频时长",
            this.$refs.video11.duration
          );
        }
      });
      return false;
    ...
    async editWorkBtn(row) {
      ...
      this.uploadCover.title_color =
        row.title_color === "ffffff" ? "白色" : "黑色";
      this.uploadCover.index_color =
        row.index_color === "ffffff" ? "白色" : "黑色";
      ...
      this.uploadVideoForm.picUrl = JSON.parse(
        JSON.stringify(Setting.url(row.vedio_cover))
      );
    ...
    confirmUpload() {
      ...
      let coverColor1 =
        this.uploadCover.title_color === "白色" ? "ffffff" : "000000";
      let coverColor2 =
        this.uploadCover.index_color === "白色" ? "ffffff" : "000000";
      let videoSS = this.time_to_sec(this.duration);
      if (this.uploadTitle === "上传视频") {
        uoloadVedioApi({
          uid: this.workUid,
          vedio_url: Setting.getSliceUrl(this.uploadVideoForm.vedio_url),
          ...
          vedio_cover: Setting.getSliceUrl(this.uploadVideoForm.picUrl),
    ...
    getWorkList(uid) {
      ...
      userWorksCopyApi({
        ...
        this.dataWorks.forEach((item) => {
          const localUrl = localStorage
            .getItem("ossUrl")
            .split(":")
            .slice(0, 2)
            .join(":");
          console.log(localUrl, "域名地址");
          const state = item.vedio_cover.indexOf("http") > -1;
          item.vedio_cover = state
            ? item.vedio_cover
            : localUrl + "/" + item.vedio_cover;
        });
        /* 加不加加不加 */
    ...
    jewelVipBtn(row) {
      ...
      this.vipUserForm.vip_end_time =
        this.vipUserForm.vip_end_time === 0
          ? "不是会员"
          : formatDate(
    ...
    priviewBtn(row) {
      ...
      const localUrl = localStorage
        ...
        .slice(0, 2)
        .join(":");
      this.playerOptions.sources[0].src = localUrl + "/" + row.vedio_url;
      console.log(Setting.url(this.playerOptions.sources[0].src), "01");
      // console.log(this.playerOptions,'notSupportedMessage');
      if (this.playerOptions.sources[0].src === "") {
        this.flagVideo = false;
      } else {
