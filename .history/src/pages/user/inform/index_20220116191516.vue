<template>
  <div class="search">
    ...
    <Form :model="searchForm" :label-width="80">
      <div class="dispose">
        <Button @click="disposeClick(3)" :type="disposeFlag===3?'primary':'default'" style="margin-right:15px">全部</Button>
        <Button @click="disposeClick('e:1')" :type="disposeFlag==='e:1'?'primary':'default'" style="margin-right:15px">待处理</Button>
        <Button @click="disposeClick('e:2')" :type="disposeFlag==='e:2'?'primary':'default'">已处理</Button>
      </div>
      <Row>
        <Col span="4">
          <FormItem label="手机号：">
        ...
        <Button type="primary" icon="ios-search" @click="searchFormBtn"
          >搜索</Button
        >
        <Button @click="sortClick('DESC')" :type="sortFlag==='DESC'?'success':'default'" style="margin:0 15px">倒序排列</Button>
        <Button @click="sortClick('ASC')" :type="sortFlag==='ASC'?'success':'default'">正序排列</Button>
        <!-- </div> -->
      </Row>
      <div>
    ...
    <Modal v-model="modalLook" width='900px'>
      ...
      <div slot="footer" style="text-align: center">
        <Button type="primary"  @click="closeBannedClick" v-if="lookMsg.complain_status===1">关闭此举报</Button>
        <Button type="primary" @click="bannedClick" v-if="lookMsg.complain_status===1">封禁</Button>
        <Button type="primary" @click="warnClick" v-if="lookMsg.complain_status===1">警告</Button>
      </div>
    </Modal>
    ...
    <Modal v-model="bannedVisible" :closable="false" width="400">
      ...
      <p> <span style="color:red">*</span> 封禁天数：<Input style="width:100px" Number v-model="blocked_day"></Input>&nbsp;天</p>
      <p
        style="
          margin-top: 20px;
          margin-bottom: 10px;
    ...
    <Modal v-model="warnVisible" :closable="false" width="400">
      <div slot="header" style="text-align: center; font-size: 18px">
       警告用户
        ...
        <p style="font-size:14px;margin-bottom:10px"> <span style="color: red"> *</span>警告理由</p>
     
      <RadioGroup
        true-value
        @on-change="WarnChange"
        v-model="season2"
        vertical
      >
...
<script>
import { informUserLisApi, informRecordApi,informhandleApi, userBanApi,cateApi } from "@/api/user";
    ...
    getRecordList(uid) {
      ...
      informRecordApi({
        ...
        this.detailTableData.forEach((item, index) => {
          item.complain_images = item.complain_images.split(",");
          item.complain_images = item.complain_images.forEach(item=>{
            item = Setting.url(item)
          })
        });
