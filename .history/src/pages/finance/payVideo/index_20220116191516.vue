        ...
        <div>
          <Row>
            <Col span="12">
              ...
              <Row>
                <Col span="7"
                  >视频总收入：<span>{{ detailForm.income_total }}/钻石</span></Col
                >
                <Col span="7"
              ...
              <Table border :columns="detailColumns" :data="detailTableData">
                <template slot-scope="{ row, index }" slot="money1">{{Math.round(Number(row.money)*100)}}</template>
              </Table>
              <!-- 分页 -->
              <div class="page">
                <Page
            ...
            <Col span="12">
              <Card>
                ...
                <p style="text-align:center;margin-top:15px"><Button type="primary" @click="closePreview">暂停播放</Button></p>
  ...
  methods: {
    ...
    getPayList() {
      ...
      payListApi({
        ...
        'e_v:is_pay':1,
        'e:is_del':0,
        'x_v:add_time': this.datePay[0]?Date.parse(this.datePay[0]) / 1000: "",
        'd_v:add_time': this.datePay[1]?Date.parse(this.datePay[1]) / 1000: "",
      })
        .then((res) => {
    ...
    detailBtn(row) {
      ...
      this.detailForm = JSON.parse(JSON.stringify(row));
       this.playerOptions.sources[0].src = this.detailForm.vedio_url;
       if(this.playerOptions.sources[0].src===''){
      this.flagVideo=false
      }else{
        this.flagVideo = true
