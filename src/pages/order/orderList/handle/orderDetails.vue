    ...
    >
      <Card :bordered="false" dis-hover class="i-table-no-border">
        ...
        <div v-if="orderDatalist.orderInfo.delivery_type === 'express'">
          ...
          <Row class="mb10" v-for="(item,index) in orderDatalist.orderInfo.deliverys" :key="index">
            <Col span="8"
              >商品名称：{{item.remark }}</Col
            >
            <Col span="8"
              >快递公司：{{item.delivery_name }}</Col
            >
            <Col span="8"
              >快递单号：{{ item.delivery_id }}
              <Button type="info" size="small" @click="openLogistics(item)"
                >物流查询</Button
              ></Col
            >
          </Row>
    ...
    >
      <div class="logistics acea-row row-top">
        ...
        <div class="logistics_cent">
          <span>物流公司：{{ params.delivery_name }}</span>
          <span>物流单号：{{ params.delivery_id }}</span>
        </div>
      </div>
      ...
      <div v-if="disply==false" style="margin:1rem 0 0 5rem;color:red;width:100% ">
          {{message}}!
      </div>
      <div v-if="logicData">
        <Steps :current="logicData.length" direction="vertical">
          <Step
            v-for="(item, index) in logicData"
            :key="index"
            :title="`${item.status}  ${item.ftime}`"
...
<script>
import { getExpress, getExpressDetail } from "@/api/order";
export default {
  name: "orderDetails",
  data() {
    return {
  ...
  methods: {
    openLogistics(item) {
      ...
      this.params.delivery_id = item.delivery_id;
      this.params.delivery_name = item.delivery_name;
      this.getOrderData();
    ...
    getOrderInfo(item) {
      getExpressDetail({
        ...
        // 'JT5059950659385',
        // item.delivery_id,
        delivery_code:  item.code,
        // 'jtexpress'
        // item.code,
      })
        .then(async (res) => {
          this.disply = res.data.result;
          this.message = res.data.message;
          this.logicData = res.data.data;
        })
        .catch((res) => {});
    },
  },
