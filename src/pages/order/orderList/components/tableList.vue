<template>
    <div>
        ...
        >
        <!-- //兑换卡列表判断 -->
            <template slot-scope="{ row, index }" slot="redeem_id">
                <span v-if="row.redeem_id!==0">兑换卡</span>
                <span v-if="row.redeem_id==0">{{row.pay_type_name}}</span>
            </template>
            ...
            <template slot-scope="{ row, index }" slot="action">
                ...
                <a @click="edit(row)"   v-if='row._status === 1'>编辑</a>
                 <Divider type="vertical"  v-if='row._status === 1'/>
                 <a @click="sendOrderV(row)" v-if="!row.deliverys&&row.pay_type_name!=='未支付'">发送货</a>
                 <a @click="deliveryV(row)" v-if="row.deliverys&&row.pay_type_name!=='未支付'">配送信息</a>
                ...
                <template>
                 ...
                 <span>
                    <!-- <a @click="edit(row)"  v-if='row._status === 1'>编辑</a>
                    <a @click="sendOrderV(row)" v-if='row._status === 2 && row.shipping_type === 1'>送货</a>
                    <a @click="delivery(row)"  v-if='row._status === 4'>配送信息</a>
                    <a @click="bindWrite(row)"  v-if="row.shipping_type == 2 && row.status == 0  && row.paid == 1 && row.refund_status === 0">立即核销</a>
        ...
        <edit-from ref="edits" :FromData="FromData" @submitFail="submitFail"></edit-from>
         <!-- 编辑 配送信息-->
         <edit-order ref="editsV" :orderId="orderId" :FromData="FromData" @submitFail="submitFail"></edit-order>
        <!-- 详情 -->
        ...
        <!-- 送货 -->
        <order-sendV ref="sendV" :orderId="orderId" @submitFail="submitFail"></order-sendV>
    </div>
</template>

<script>
    import expandRow from './tableExpand.vue';
    import {getExpressDetail, orderList, getOrdeDatas, getDataInfo, getRefundFrom, getnoRefund, refundIntegral, getDistribution, writeUpdate } from '@/api/order';
    import { mapState, mapMutations } from 'vuex';
    import editFrom from '../../../../components/from/from';
    import editOrder from '../handle/editOrder'
    ...
    import orderSendV from '../handle/orderSendV';
import EditOrder from '../handle/editOrder.vue';

    export default {
        name: 'table_list',
        components: { expandRow, editFrom,editOrder, detailsFrom, orderRemark, orderRecord, orderSend,orderSendV, EditOrder },
        data () {
            return {
                delfromData: {},
                modal: false,
        ...
        created () {
            this.getList();
            window.localStorage.setItem('pageData', this.page.pageNum)
            window.localStorage.setItem('pageDataLimit', this.page.pageSize)
        },
        watch: {
            'orderType': function () {
                this.page.pageNum = 1;
        ...
        methods: {
            ...
            getData (id) {
                getDataInfo(id).then(async res => {
                    ...
                    console.log(this.orderDatalist,'jjjjj')
            ...
            //送货
            sendOrderV (row){
                this.$refs.sendV.modals = true;
                this.$refs.sendV.getList();
                // this.$refs.sendV.getDeliveryList();
                this.orderId = row.id;
            ...
            //配送信息
             deliveryV (row) {
                 this.orderId = row.id;
                 this.$refs.editsV.getList();
                 this.$refs.editsV.modals = true;
                 this.$refs.editsV.orderData = row.deliverys
                //  console.log(row.deliverys,'row.deliverys')
                //  console.log(row,'plplpl')
