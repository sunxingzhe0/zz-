<template>
  <div class="">
    ...
    <div class="search">
      ...
      <div style="margin-left: 20px">
        <span>排列方式：</span>
        <Select v-model="model2" @on-change="sortChange" style="width: 120px">
          <Option
      ...
      <Table border :columns="activeColumns" :data="activeData">
        <!-- 视频付费用户数/日活跃用户数 -->
         <template slot-scope="{ row, index }" slot="result">
          <span v-if="row.count_active!=0">{{((row.count_vedio_pay / row.count_active)*100).toFixed(2)}}%</span>
          <span v-else>0.00%</span>
        </template>
        <!-- 充值用户数/日活跃用户数 -->
        <template slot-scope="{ row, index }" slot="result2">
          <span v-if="row.count_active!=0">{{((row.count_recharge / row.count_active)*100).toFixed(2)}}%</span>
          <span v-else>0.00%</span>
        </template>
        <!-- 充值总金额/充值用户数 -->
        <template slot-scope="{ row, index }" slot="result3">
          <span v-if="row.count_recharge!=0">{{((row.time_recharge / row.count_recharge)).toFixed(2)}}</span>
          <span v-else>0.00</span>
        </template>
             <!-- 充值总金额/日活跃用户数 -->
        <template slot-scope="{ row, index }" slot="result4">
          <span v-if="row.count_active!=0">{{((row.time_recharge / row.count_active)).toFixed(2)}}</span>
          <span v-else>0.00</span>
        </template>
        <!-- 跳转查看count_recharge -->
        <template slot-scope="{ row, index }" slot="roter">
          <p>{{row.count_recharge}}</p>
          <span @click="routerRow(row)" class="roterSearch">查看</span> 
        </template>
      </Table>
      <!-- 分页 -->
      <div class="page">
...
<script>
import { payListApi,chanelNAmeListApi,hanfSelfTotalApi } from "@/api/active";
...
export default {
  data() {
    return {
      ...
      activeColumns: [
        ...
        {
          title: "充值用户平均充值金额ARPPU值",
          slot:"result3",
          align: "center",
          className: "table-head-style",
        },
        {
          title: "活跃用户平均充值金额ARPPU值",
          slot:"result4",
          align: "center",
          className: "table-head-style",
        },
  ...
  methods: {
    ...
    chanelChange(e){
      this.modelChanel = e
      console.log(this.modelChanel,'iiii');
      this.getPayRateLis()
    },
    //获取付费率充值率接口
    getPayRateLis() {
    ...
    dayChange(e) {
      this.valueDay = e;
      console.log(this.valueDay,'this.valueDay');
      this.getPayRateLis()
      // this.spinShow = true
    },
    //排列方式change事件
    ...
    routerRow(row){
      ...
      let timeRouter = [];
      timeRouter[0] = datestr?datestr:""
      timeRouter[1] = datestr?datestr:""
    //  console.log(timeRouter,'timeRouter');
      this.$router.push({path:'/admin/finance/recharge',query:{timeRouter:timeRouter}})
    },
    // 手动统计
    nowTotalClick(num){
      this.spinShow = true
