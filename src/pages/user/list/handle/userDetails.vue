    ...
    >
      ...
      <Row type="flex" justify="space-between" class="mt25">
        ...
        <Col span="20">
          ...
          >
           <template slot-scope="{ row, index }" slot="actionDate">
           {{row.start_time}}~{{row.stop_time}}
        </template>
          <template slot-scope="{ row, index }" slot="actionShopping">
           {{row.store_name}}
        </template>
          <template slot-scope="{ row, index }" slot="actionOpen">
            <span type="border" v-if="row.isopen==1">已开启</span>
            <span type="border" v-if="row.isopen==0">未开启</span>
        </template>
         <template slot-scope="{ row, index }" slot="actionState">
            <span type="border" v-if="row.status==0">未使用</span>
            <span type="border" v-if="row.status==1">已兑换</span>
            <span type="border" v-if="row.status==2">已兑物</span>
        </template>
           <template slot-scope="{ row, index }" slot="actionOid">
            <span v-if="row.oid!==0">{{row.oid}}</span>
            <span v-if="row.oid==0"></span>
        </template>
          </Table>
  ...
  data() {
    return {
      ...
      list: [
        ...
        { val: "redeem1", label: "兑换卡记录" },
        { val: "redeem2", label: "兑换卡兑换记录" },
