    ...
    <Card :bordered="false" dis-hover class="ivu-mt listbox">
            ...
            <div style="padding-left: 28px;">
                ...
                <Col span="24">
                    <Col v-bind="grid">
                        ...
                        <Col span="11">
                        <div class="line">
                            <div class="text-left">购买次数</div>
                            <Input
                            number
                            clearable
                            class="input"
                            placeholder="请输入最低次数"
                            v-model="exchangeFrom.mincount"
                            />
                        </div>
                        </Col>
                        <Col span="1" class="connect"> — </Col>
                        ...
                        <div class="line">
                            ...
                            class="input"
                            placeholder="请输入最高次数"
                            v-model="exchangeFrom.maxcount"
                                @on-blur="countMaxBlur"
                            />
                        </div>
                        </Col>
                    </Row>
      ...
      >
        ...
        <template slot-scope="{ row, index }" slot="statu">
          <Tag  color="blue" v-if="row.status=='正常'">启用</Tag>
           <Tag  color="blue" v-if="row.status!=='正常'">停用</Tag>
        </template>
...
export default {
  ...
  methods: {
    ...
    //最大次数、最小次数
    countMaxBlur() {
      let maxCount = parseInt(this.exchangeFrom.maxcount);
      let minCount = parseInt(this.exchangeFrom.mincount);
      
      // console.log(maxMoney,'金额')
      if (maxCount < minCount) {
        this.$Message.warning("最大值不能小于最小值哦！");
        this.exchangeFrom.maxcount = "";
      }
    },
    // 标签弹窗关闭
    labelClose() {
    ...
    //导出
     exportTable() {
           ...
           this.userFrom.field_key = this.field_key === "all" ? "" : this.field_key;
            ...
            let maxCount = this.exchangeFrom.maxcount==""?"":parseInt(this.exchangeFrom.maxcount);
            let minCount = this.exchangeFrom.mincount==""?"":parseInt(this.exchangeFrom.mincount);
      this.$Modal.confirm({
          title: '确定要导出列表数据吗？',
          onOk: () => {
                 
              
           window.location.href  =`${location.origin}/adminapi/user/user?`+"export="+1
           +"&user_type="+this.userFrom.user_type +
            "&status="+ this.userFrom.status +"&nickname="+this.userFrom.nickname+"&page="+this.userFrom.page+"&limit="+
            this.userFrom.limit+"&field_key="+this.userFrom.field_key+"&pay_count_min="+minCount+
            "&pay_count_max="+maxCount+"&now_money_min="+minMoney+"&now_money_max="+maxMoney;
            console.log(aaa,this.userFrom.status,'kkkkk')
    ...
    //   this.userFrom.group_id = this.group_id === "all" ? "" : this.group_id;
      ...
      userList({
          ...this.userFrom,
          pay_count_min:minCount,
          pay_count_max:maxCount,
          now_money_min:minMoney,
          now_money_max:maxMoney
      })
        .then(async (res) => {
          let data = res.data;
