<template>
    <div>
        ...
        <Card :bordered="false" dis-hover class="ivu-mt">
            <Form ref="formValidate" :model="formValidate"  :label-width="labelWidth" :label-position="labelPosition" @submit.native.prevent>
                <Row type="flex" :gutter="24">
                    ...
                    <Col v-bind="grid">
                        <FormItem label="部门名称："  label-for="role_name">
                            <Input search enter-button placeholder="请输入部门名称" v-model="formValidate.role_name" @on-search="userSearchs"/>
                        </FormItem>
                    </Col>
                </Row>
                <Row type="flex">
                    <Col v-bind="grid">
                            <Button v-auth="['setting-system_role-add']" type="primary"  icon="md-add" @click="add('添加')">添加部门</Button>
                    </Col>
                </Row>
            </Form>
        ...
        <!-- 新增编辑-->
        <Modal v-model="modals" @on-cancel="onCancel" scrollable  footer-hide closable :title="`${modelTit}部门`" :mask-closable="false"  width="600">
            <Form ref="formInline" :model="formInline" :rules="ruleValidate"  :label-width="100" :label-position="labelPosition2" @submit.native.prevent>
                <FormItem label="部门名称："  label-for="role_name" prop="role_name">
                    <Input  placeholder="请输入部门名称" v-model="formInline.role_name"/>
                </FormItem>
