  ...
  <div class="search">
    ...
    <Form :model="searchForm" :label-width="80">
      <Row>
        ...
        <!-- <Col span="8">
          <FormItem label="充值状态：">
            <Select
              @on-change="statusIsChange"
              v-model="searchForm['e_m:recharge_status']"
            >
              <Option
    ...
    <div class="table">
      <Table border :columns="payColumns" :data="payTableData">
        <!-- 支付金额   -->
        <template slot-scope="{ row, index }" slot="payMoneySlot">
          <span  v-if="row.type==1">---</span>
          <span  v-if="row.type==0">{{  row.price}} </span>  
        </template>
        <!-- 兑换钻石数 -->
        <template slot-scope="{ row, index }" slot="changeZSSlot">
          <span  v-if="!row.r_money">{{ Math.round((row.money)*100) }} </span>
          <span  v-if="row.r_money">{{  Math.round(row.r_money)}} </span>
        </template>
        <!-- 充值类型 -->
         <template slot-scope="{ row, index }" slot="recharge_type">
            <span v-if="row.type==1">后台充值</span>
            <span v-if="row.type==0">自行充值</span>
        </template>
        <!-- 充值状态 -->
         <!-- <template slot-scope="{ row, index }" slot="recharge_status">
            <span v-if="row.recharge_status===1">充值中</span>
            <span v-if="row.recharge_status===2">充值成功</span>
            <span v-if="row.recharge_status===3">充值失败</span>
        </template> -->
         <!-- 支付状态 -->
         <template slot-scope="{ row, index }" slot="r_staus">
            <span v-if="row.status===0">待支付</span>
            <span v-if="row.status===1">已支付</span>
            <!-- <span v-if="row.order_status===3">退款</span> -->
        </template>
        <!-- 操作 -->
    ...
    <Modal v-model="modalLook" :closable="false" width="400">
      ...
      <p class="title">状态：<span v-if="detailForm.status===0">待支付</span>
      <span v-if="detailForm.status===1">已支付</span>
      <!-- <span v-if="detailForm.r_staus===3">已退款</span> -->
      </p>
      ...
      <Row>
        <Col span="12">充值时间<span>{{detailForm.add_time}}</span></Col>
      </Row>
      <Row>
        <Col span="12">支付金额：<span  v-if="detailForm.type==1">------ </span>
          <span  v-if="detailForm.type==0">{{ detailForm.price}} </span></Col>
        <Col span="12">兑换钻石数： {{detailForm.money}} </Col>
      </Row>
      <div v-if="detailForm.order_status===3">
          <p class="title">退款信息</p>
      <Row>
        <Col span="12">退款到账：<span>{{detailForm.money}}</span></Col>
      </Row>
      <Row>
    ...
    <Modal v-model="modalReturn" :closable="false" width="400">
      ...
      <p class="title">支付信息</p>
      <Row>
        <Col span="12">充值时间：<span>{{returnForm.add_time}}</span></Col>
        <Col span="12">支付金额：<span>{{returnForm.money}}</span></Col>
      </Row>
      <Row>
        <Col span="12">兑换钻石数：<span>{{parseInt(returnForm.money)*100}}</span></Col>
      </Row>
      <p class="title">退款信息：<span>{{returnForm.nickname}}</span></p>
      <Row>
        <Col span="12">退款到账：<span>{{returnForm.money}}</span></Col>
      </Row>
      <Row>
        <Col span="12">退还钻石数：<span>{{parseInt(returnForm.money)*100}}</span></Col>
      </Row>
...
<script>
import { rechargeRecordNewApi,payRecordReturnApi } from "@/api/finance";
...
export default {
  ...
  created() {
   this.routerDates =this.$route.query.timeRouter?this.$route.query.timeRouter:""
   console.log(this.routerDates,'routerDates');
    this.datePay =  this.routerDates
    // console.log(this.datePay,'跳转事件');
    // console.log( this.$route.query.timeRouter,'testArray');
  ...
  methods: {
    //获取充值记录列表
    getRechargeList() {
      rechargeRecordNewApi({
        ...this.exchangeFrom,
        ...this.searchForm,
        "x:r:create_time": this.datePay[0]? this.datePay[0]+" 00:00:00":"",
        "d:r:create_time": this.datePay[1]? this.datePay[1]+" 23:59:59":"",
      }).then((res) => {
    ...
    //支付状态change事件
    orderIsChange(e) {
      this.searchForm["e:r:status"] = e;
      this.getRechargeList();
    },
    //充值状态
    statusIsChange(e) {
      this.searchForm["e_m:recharge_status"] = e;
      this.getRechargeList();
    },
    //充值类型
    typeIsChange(e) {
      this.searchForm["e:r:type"] = e;
      this.getRechargeList();
    },
    //退款
    returnBtn(row) {
